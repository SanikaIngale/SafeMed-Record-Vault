...
// ── Icons ─────────────────────────────────────────────────────────────────────
const PersonIcon = () => (<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>);
const MailIcon   = () => (<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>);
const PhoneIcon  = () => (<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.28 2 2 0 0 1 3.59 1h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 8.96a16 16 0 0 0 6.13 6.13l1.32-1.32a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>);
...
const IdIcon     = () => (<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="8" y1="10" x2="8" y2="10" strokeWidth="3"/><line x1="12" y1="9" x2="16" y2="9"/><line x1="12" y1="12" x2="16" y2="12"/><line x1="12" y1="15" x2="14" y2="15"/><circle cx="8" cy="10" r="1.5" fill="currentColor"/></svg>);
const HospitalIcon = () => (<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9,22 9,12 15,12 15,22"/></svg>);
const CalendarIcon = () => (<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>);
...
const specialtyOptions = [
  ...
  "General Medicine","Internal Medicine","Diabetes Management","Preventive Healthcare",
];
    ...
    <div style={{ marginBottom: error ? "8px" : "20px" }}>
      ...
      {selected.length > 0 && (
        <div style={{ display:"flex", flexWrap:"wrap", gap:"6px", marginBottom:"8px" }}>
          {selected.map(s => (
            <span key={s} style={{ display:"inline-flex", alignItems:"center", gap:"6px", padding:"5px 10px", borderRadius:"999px", background:`${palette.primary}18`, border:`1.5px solid ${palette.primary}`, color:palette.primary, fontSize:"13px", fontFamily:fonts.body, fontWeight:"600" }}>
              {s}
              <button onClick={() => remove(s)} style={{ background:"none", border:"none", cursor:"pointer", color:palette.primary, padding:"0", lineHeight:1, fontSize:"15px", display:"flex", alignItems:"center" }}>×</button>
            </span>
          ))}
        </div>
      )}
      <div style={{ position:"relative" }}>
        <button onClick={() => setOpen(o => !o)} style={{ width:"100%", padding:"13px 14px", background:palette.inputBg, border:`1.5px solid ${error ? palette.error : palette.border}`, borderRadius: open ? `${radius.md} ${radius.md} 0 0` : radius.md, fontSize:"14px", color:palette.textSecondary, fontFamily:fonts.body, outline:"none", cursor:"pointer", textAlign:"left", display:"flex", justifyContent:"space-between", alignItems:"center", boxSizing:"border-box" }}>
        ...
        {open && (
          <div style={{ position:"absolute", top:"100%", left:0, right:0, zIndex:50, background:palette.inputBg, border:`1.5px solid ${palette.border}`, borderTop:"none", borderRadius:`0 0 ${radius.md} ${radius.md}`, maxHeight:"220px", overflowY:"auto", boxShadow:"0 8px 24px rgba(0,0,0,0.12)" }}>
            {specialtyOptions.map(s => {
              const isSel = selected.includes(s);
              return (
                <div key={s} onClick={() => toggle(s)} style={{ padding:"11px 14px", cursor:"pointer", fontSize:"14px", fontFamily:fonts.body, display:"flex", alignItems:"center", gap:"10px", background: isSel ? `${palette.primary}10` : "transparent", color: isSel ? palette.primary : palette.textPrimary, fontWeight: isSel ? "600" : "400" }}>
                  <span style={{ width:"16px", height:"16px", borderRadius:"4px", flexShrink:0, border:`2px solid ${isSel ? palette.primary : palette.border}`, background: isSel ? palette.primary : "transparent", display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      <div style={{ display:"flex", gap:"8px", marginTop:"8px" }}>
        <input type="text" placeholder="Can't find yours? Type a custom specialization…" value={customInput} onChange={e => setCustomInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); addCustom(); }}}
          style={{ flex:1, padding:"10px 12px", background:palette.inputBg, border:`1.5px solid ${palette.border}`, borderRadius:radius.md, fontSize:"13px", color:palette.textPrimary, fontFamily:fonts.body, outline:"none", boxSizing:"border-box" }} />
        <button onClick={addCustom} disabled={!customInput.trim()} style={{ padding:"10px 14px", borderRadius:radius.md, background: customInput.trim() ? palette.primary : palette.border, border:"none", color: customInput.trim() ? "#fff" : palette.textSecondary, fontSize:"13px", fontFamily:fonts.body, fontWeight:"600", cursor: customInput.trim() ? "pointer" : "not-allowed", whiteSpace:"nowrap" }}>+ Add</button>
      </div>
  ...
  <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"28px" }}>
    {Array.from({ length: total }).map((_, i) => (
      <React.Fragment key={i}>
        <div style={{ width: i === current ? "28px" : "10px", height:"8px", borderRadius:"999px", background: i <= current ? palette.primary : palette.border, transition:"all 0.3s ease", flexShrink:0 }} />
      </React.Fragment>
    ))}
    <span style={{ marginLeft:"auto", fontSize:"12px", color:palette.textSecondary, fontFamily:fonts.body }}>Step {current + 1} of {total}</span>
  </div>
);

const FieldLabel = ({ children }) => (
  <label style={{ display:"block", fontSize:"11px", fontWeight:"700", color:palette.textSecondary, marginBottom:"6px", letterSpacing:"1px", textTransform:"uppercase", fontFamily:fonts.body }}>{children}</label>
);
...
const CredentialCard = ({ cred, index, onChange, onRemove, errors }) => {
  ...
  const inputStyle = (errKey) => ({
    width:"100%", padding:"11px 12px", background:palette.inputBg,
    border:`1.5px solid ${errors?.[errKey] ? palette.error : palette.border}`,
    borderRadius:radius.md, fontSize:"14px", color:palette.textPrimary,
    fontFamily:fonts.body, outline:"none", boxSizing:"border-box",
  });

  return (
    <div style={{ background:palette.inputBg, border:`1.5px solid ${palette.border}`, borderRadius:radius.md, padding:"16px", marginBottom:"12px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"14px" }}>
        <span style={{ fontSize:"13px", fontWeight:"700", color:palette.textPrimary, fontFamily:fonts.body }}>Credential {index + 1}</span>
        {index > 0 && (
          <button onClick={() => onRemove(index)} style={{ background:"none", border:"none", cursor:"pointer", color:palette.error, padding:"4px", borderRadius:"4px", display:"flex", alignItems:"center" }}><TrashIcon /></button>
        )}
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"10px" }}>
        ...
        <div>
          ...
          <input type="number" min="1950" max={new Date().getFullYear()} placeholder="e.g. 2010" value={cred.year} onChange={field("year")} style={inputStyle("year")} />
      ...
      <div style={{ marginBottom:"10px" }}>
        ...
        <input type="text" placeholder="e.g. General Medicine, Cardiology…" value={cred.field} onChange={field("field")} style={inputStyle("field")} />
        {errors?.field && <p style={{ fontSize:"11px", color:palette.error, marginTop:"3px" }}>{errors.field}</p>}
      </div>
      <div>
        <FieldLabel>Institution</FieldLabel>
        <input type="text" placeholder="e.g. AIIMS, Delhi" value={cred.institution} onChange={field("institution")} style={inputStyle("institution")} />
...
const SignUpPage = () => {
  ...
  const [form, setForm] = useState({ fullName:"", email:"", phone:"", dob:"", licenseId:"", hospital:"", specialties:[], password:"", confirmPassword:"" });
  const [credentials, setCredentials] = useState([{ type:"", field:"", institution:"", year:"" }]);
  ...
  const updateCred = (idx, key, val) => {
    ...
    if (credErrors[idx]?.[key]) setCredErrors(ce => ce.map((e, i) => i === idx ? { ...e, [key]: undefined } : e));
  };
  const addCred    = () => { setCredentials(cs => [...cs, { type:"", field:"", institution:"", year:"" }]); setCredErrors(ce => [...ce, {}]); };
  const removeCred = (idx) => { setCredentials(cs => cs.filter((_, i) => i !== idx)); setCredErrors(ce => ce.filter((_, i) => i !== idx)); };
  ...
  const handleBack = () => { setErrors({}); setApiError(""); setStep(s => s - 1); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        credentials: credentials.map(c => ({ type: c.type, field: c.field, institution: c.institution, year: c.year })),
      ...
      // Persist token + doctor info for ProfilePage
  ...
  return (
    <AuthLayout maxWidth="460px">
      ...
      {step === 2 && (
        <>
          ...
          {credentials.map((cred, i) => (
            <CredentialCard key={i} cred={cred} index={i} onChange={updateCred} onRemove={removeCred} errors={credErrors[i]} />
          ))}
          <button onClick={addCred} style={{ width:"100%", padding:"12px", border:`1.5px dashed ${palette.primary}`, borderRadius:radius.md, background:`${palette.primary}08`, color:palette.primary, fontSize:"14px", fontFamily:fonts.body, fontWeight:"600", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"8px", marginBottom:"20px" }}>
        ...
        <>
          ...
          {form.password && (
            <div style={{ marginBottom:"16px" }}>
              ...
              <p style={{ fontSize:"11px", color:palette.textSecondary, fontFamily:fonts.body }}>Use uppercase, numbers, and symbols to strengthen your password.</p>
            </div>
          )}

          <div style={{ display:"flex", alignItems:"flex-start", gap:"10px", marginBottom: errors.agreed ? "8px" : "24px" }}>
            <input type="checkbox" id="terms" checked={agreed} onChange={e => { setAgreed(e.target.checked); if (errors.agreed) setErrors(er => ({ ...er, agreed: undefined })); }} style={{ width:"18px", height:"18px", accentColor:palette.primary, marginTop:"2px", flexShrink:0, cursor:"pointer" }} />
            <label htmlFor="terms" style={{ fontSize:"13px", color:palette.textSecondary, fontFamily:fonts.body, lineHeight:1.5, cursor:"pointer" }}>
              I agree to the{" "}
              <Link to="/terms" style={{ color:palette.primary, fontWeight:"600" }}>Terms of Service</Link>{" "}and{" "}
      ...
      <div style={{ display:"flex", gap:"12px", marginTop:"8px", marginBottom:"24px" }}>
        {step > 0 && <Button variant="outline" onClick={handleBack} style={{ flex:1, padding:"14px" }}>← Back</Button>}
        {step < 3
      ...
      <p style={{ textAlign:"center", fontSize:"14px", color:palette.textSecondary, fontFamily:fonts.body }}>
        ...
        <Link to="/login" style={{ color:palette.primary, fontWeight:"700", textDecoration:"none", borderBottom:`1.5px solid ${palette.primary}` }}>Sign in</Link>
      </p>
    </AuthLayout>
  );
};
